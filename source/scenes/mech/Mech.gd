...
func think_move():
	...
	# Step 0: Pick offensive or defensive weights for position evaluations
	var weights = ai_standard.offense
	if (armLHP <= mechData.arm_l.hp * 0.5 && armRHP <= mechData.arm_r.hp * 0.5) || bodyHP <= mechData.body.hp * 0.5:
		weights = ai_standard.defense
	elif (armLHP <= 0 && armRHP <= 0) || bodyHP <= mechData.body.hp * 0.25:
		weights = ai_standard.retreat
	
	# Step 1: Update unit list with target, aggro, distance, and range info
	# Designate primary target and primary threat? ie, Who do I hate most and who do I fear most?
	for unit in unit_list:
		# Reset target level
		unit.target = 1.0
		unit.aggro = 0.0
		if !unit.mech.is_dead:
			unit.target += 1.0 * (1.0 - (unit.mech.bodyHP / unit.mech.mechData.body.hp))
			unit.target += 1.0 * (1.0 - (unit.mech.armRHP / unit.mech.mechData.arm_r.hp))
			unit.target += 1.0 * (1.0 - (unit.mech.armLHP / unit.mech.mechData.arm_l.hp))
			unit.target += 1.0 * (1.0 - (unit.mech.legsHP / unit.mech.mechData.legs.hp))
			unit.aggro += unit.last_dmg / unit.last_attack
	
	# Step 2: Go through movement squares and calculate position value
	update_wpn()
	priority_list.clear()
	var priority = 0
	#var unit_tile = null
	for tile in move_tiles:
		...
		for unit in unit_list:
			...
			if (!unit.friend && !unit.mech.is_dead):
				unit_range = get_range(tile, unit.mech.curr_tile)
				# How far are each of my enemies to this square?
				priority += unit.target * (move_range / unit_dist) * weights.enemy
				if tile.get_los(unit.mech.curr_tile):
					# How many enemies can I hit per weapon from this square?
					for weapon in weapon_list:
						if weapon.active && (unit_range >= weapon.range_min && unit_range <= weapon.range_max):
							priority += unit.target * weights.enemy
					# How many enemy weapons can hit this square?
					var in_range_count = 0
					for weapon in unit.mech.weapon_list:
						if weapon.active && (unit_range >= weapon.range_min && unit_range <= weapon.range_max):
							in_range_count += 1
					priority += (16 - in_range_count) * weights.cover
				elif (!unit.friend && !unit.mech.is_dead):
					priority += (move_range / unit_dist) * weights.allies
		for item in item_list:
			# Is there a repair kit on this tile?
			# - +1 if repair kit on tile, 0 otherwise
			# - Scale by AI state's weight
			if item.is_in_group("repair"):
				var item_tile = item.curr_tile.index
				if item_tile == tile.index:
					priority += weights.repair
				elif nav_paths[item_tile].distance > 0:
					priority += weights.repair / nav_paths[item_tile].distance
			# Is this tile in an explosion radius?
			# - +1 if not in radius, 0 otherwise
			# - Scale by AI state's weight
			if item.is_in_group("bomb"):
				priority += get_range(tile, item.curr_tile) * weights.splash
...
func think_action():
	...
	var unit_range = 0
	unit_list.sort_custom(CustomSort, "target")
	for unit in unit_list:
